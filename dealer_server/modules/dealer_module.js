...
class user_module extends base_module
{
	...
	service(){
		...
		this.app.get('/get_all_dealers',function(req,res,next){//zyh
			this.get_all_dealers(req,res);
		}.bind(this));
...
//====================================================================================
	...
	get_all_dealers(req,res){//zyh
		var token = req.query.token;
		var account = req.query.account;
		var sql = 'SELECT * FROM t_dealers';
		var ret = this.query(sql);
		if(ret.err){
			console.log(ret.err);
			return null;
	...
	dealer_tree={};//zyh 
	create_null_dealer_node(dealer){//zyh 
		dealer_tree[dealer.token]={
			data:dealer,
			parent:null,
			name:"",
			child:[],
	...
	create_dealer_tree_node(dealer,parent,child){	//zyh 
		if(dealer == null && parent!=null && child!=null){
			dealer_tree[parent]={
				data:dealer,
				parent:null,
				name:"",
			...
			dealer_tree[parent].child[child]=true;
			return;
		}
		if(dealer!=null){
			var token = dealer.token ;
			var parent = dealer.parent;
			var name = dealer.name;
			if(dealer_tree[token] == null) create_null_dealer_node(dealer);
			dealer_tree[token].parent = parent;
			dealer_tree[token].name = name;
			if(child!=null ||typeof(child) =="string") dealer_tree[token].child[child] = true;
			if(parent==null ||typeof(parent) !="string") return;
			create_dealer_tree_node(dealer_tree[parent],parent,token);
		}
	}

	get_dealer_tree(req,res){//zyh 
		var row = this.get_all_dealers(req,res);
		for(var i=0;i<row;++i){
			var dealer = row[i];
			create_dealer_tree_node(dealer);
		}
		return dealer_tree;
	};


	get_sub_dealer_list(dealer){//zyh 
		var dealer_name_list=[];
		var tmp = dealer_tree[dealer.token];
		for(var k in tmp.child){
			if(typeof(k)=="string"){
				dealer_name_list.push[dealer_tree[k].name];
			}
		}
		return dealer_name_list;
	}
}
