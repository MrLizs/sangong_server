...
sanGongManager.prototype = {
    ...
    getStatePushData:function(game){
        return {
            ...
            gameLength:game.roomInfo.conf.maxGames,
            gameTime:game.time,
    ...
    stake: function (userId, stakeInfo) {
        ...
        if(game.roomInfo.userMap[userId].yuanbaos<staked+stake){
            userMgr.sendMsg(userId,'stake_notify_push', {error:'noYuanbaos'});
            return;
        }
        if (game.roomInfo.userMap[userId].yuanbaos > 0 && game.roomInfo.userMap[userId].yuanbaos >= staked + stake) {
            ...
            userMgr.broacast(roomId, 'user_yuanbaos_notify_push', { userid: userId, yuanbaos: game.roomInfo.userMap[userId].yuanbaos - game.userStakeMap[userId], yuanbaosbank: game.roomInfo.userMap[userId].yuanbaosbank });
        }
    },
    ...
    enterGame: function (userId) {
        ...
        if (!game) {
            var data = {
                ...
                remainingGames: roomInfo.conf.maxGames,
                state: 'stop',
    ...
    startGame: function (userId) {
        ...
        if(roomInfo.conf.creator!=userId){
            return;
        }
        ...
        if (game) {
            ...
            for (var i = 0; i < game.gameSeats.length; i++) {//zyh
                db.user_online_log(game.gameSeats[i].userId,1,function(res){
    ...
    stopGame: function (userId) {
        ...
        for (var i = 0; i < game.gameSeats.length; i++) {//zyh
            db.user_online_log(game.gameSeats[i].userId,0,function(res){
                if(!res) console.log(" user_online_log has err");
            }); 
        }
    },
    ...
    gameBegin: function (roomId) {
        ...
        //如果是第一次，则扣除房卡
        if (game.roomInfo.numOfGames == 0&&game.roomInfo.conf.cost>0) {
            var cost = game.roomInfo.conf.cost;
    ...
    gameShowResult: function (game) {
        ...
        for (var i = 0; i < seatsData.length; i++) {
            ...
            for (var userId in seatData.stakeMap) {
                ...
                if (stakeInfo && stakeInfo.total > 0) {
                    ...
                    if (scoreResult > 0) {
                        db.add_commissionhistory(userId,'gamegold',scoreResult * rate ,function(err,row){if(row){console.log(row);};});
        ...
        console.warn('-----------------------------');
        game.roomInfo.results.push(userScoreMap);//保存总成绩
        var balanceInfos=[];
        for(var userId in userScoreMap){
            var user = game.roomInfo.userMap[userId];
            if (user) {
                user.yuanbaos += userScoreMap[userId].result;
                //userMgr.sendMsg(userId, 'user_coins_notify_push', { coins: user.coins, coinsbank: user.coinsbank }); 
                userMgr.broacast(game.roomInfo.id, 'user_yuanbaos_notify_push', {userid:userId, yuanbaos: user.yuanbaos, yuanbaosbank: user.yuanbaosbank }); 
                if(user.yuanbaos==null){
                    console.error('ERROR:gameShowResult user_yuanbaos_notify_push yuanbaos==null. ==>>');
                    console.error('userScoreMap['+userId+']');
                    console.error(userScoreMap[userId]);
                    console.error('userMap['+userId+']');
                    console.error(user);
                }
                var balanceInfo = {
                    userId: userId, 
        ...
        //保存游戏
        db.create_game(game.roomInfo.uuid, game.gameIndex, self.buildGameBaseInfoStr(game), function(ret){
            db.update_game_result(game.roomInfo.uuid, game.gameIndex, userScoreMap);
            //记录玩家操作
        ...
        //游戏存档
        db.archive_games(game.roomInfo.uuid);
       
        var isEnd = game.roomInfo.numOfGames>=game.roomInfo.conf.maxGames;
        if (isEnd) {
            game.isRunning = false;
            for (var i = 0; i < game.roomInfo.users.length; i++) {
                var user = game.roomInfo.users[i];
                ...
                var results = [];
                var total = 0;
                for(var j = 0;j<game.roomInfo.results.length;j++){
                    var userScoreMap = game.roomInfo.results[j];
                    if(userScoreMap[userId]){
                        results.push(userScoreMap[userId].result);
                        total+=userScoreMap[userId].result;
                    }else{
                        results.push(0);
                    }
                }
                userMgr.sendMsg(userId,'game_over_push', { results:results,total:total,yuanbaos:user.yuanbaos });
            }
            console.warn('ALL GAME OVER room['+game.roomInfo.id+'] results==>>');
            console.warn(JSON.stringify(game.roomInfo.results));
            console.warn('-----------------------------');
            //userMgr.broacast(game.roomInfo.id,'game_over_push', { endinfo: {} });
            var gameManager = self;
            setTimeout(function () {
                delete gameManager.games[game.roomInfo.id];
                userMgr.kickAllInRoom(game.roomInfo.id);
                roomMgr.destroy(game.roomInfo.id);
            }, 1000);
        }
        ...
        if (user) {
            ...
            db.bank_save_coins(userId, coins, function (err, userInfo) {
                if (err) {
                    ...
                    userMgr.broacast(roomId, 'bank_savecoins_notify_push', { userId: userId, coins: userInfo.coins, coinsbank: user.coinsbank });
        ...
        db.bank_draw_coins(userId, coins, function (err, userInfo) {
            if (err) {
                ...
                if (user) {
                    ...
                    user.yuanbaos = userInfo.yuanbaos;
                    user.yuanbaosbank = userInfo.yuanbaosbank;
                    userMgr.broacast(roomId, 'bank_drawcoins_notify_push', { userId: userId, coins: userInfo.coins, coinsbank: user.coinsbank });
                    userMgr.broacast(roomId, 'user_coins_notify_push', { userid: userId, coins: user.coins, coinsbank: user.coinsbank });
                } else {
        ...
        if (user) {
            ...
            db.bank_save_yuanbaos(userId, yuanbaos, function (err, userInfo) {
                if (err) {
                    ...
                    user.yuanbaos = userInfo.yuanbaos;
                    user.yuanbaosbank = userInfo.yuanbaosbank;
                    userMgr.broacast(roomId, 'bank_saveyuanbaos_notify_push', { userid: userId, yuanbaos: user.yuanbaos - staked, yuanbaosbank: user.yuanbaosbank });
                    userMgr.broacast(roomId, 'user_yuanbaos_notify_push', { userid: userId, yuanbaos: user.yuanbaos - staked, yuanbaosbank: user.yuanbaosbank });
                }
            });
        } else {
        ...
        db.bank_draw_yuanbaos(userId, yuanbaos, function (err, userInfo) {
            if (err) {
                ...
                if (user) {
                    ...
                    user.yuanbaos = userInfo.yuanbaos;
                    user.yuanbaosbank = userInfo.yuanbaosbank;
                    userMgr.broacast(roomId, 'bank_drawyuanbaos_notify_push',{ userid: userId, yuanbaos: user.yuanbaos - staked, yuanbaosbank: user.yuanbaosbank });
                    userMgr.broacast(roomId, 'user_yuanbaos_notify_push', { userid: userId, yuanbaos: user.yuanbaos - staked, yuanbaosbank: user.yuanbaosbank });
                } else {
    ...
    tip:function(userId){
        ...
        if (!(roomInfo.conf.tipYuanbaos>0)) {
            userMgr.sendMsg(userId,'tip_notify_push',{error:'tip closed'});
            return;
        }
        var tip = roomInfo.conf.tipYuanbaos;
        ...
        if(user.yuanbaos-staked<tip){
            userMgr.sendMsg(userId,'tip_notify_push',{error:'no enough yuanbaos'});
            return;
        }
        if (user.yuanbaos - staked >= tip) {
            db.update_user_yuanbaos(userId, -tip, function (err) {
                if (err) {
                    userMgr.sendMsg(userId, 'tip_notify_push', { error: 'update_user_yuanbaos err' });
                    return;
                } else {
                    user.yuanbaos -= tip;
